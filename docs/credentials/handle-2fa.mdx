...
<Note>
Skyvern generates TOTP codes using standard 30-second windows. If a code has less than 20 seconds remaining, Skyvern waits for the next code to ensure successful entry.
</Note>
...
When your automation needs a 2FA code, Skyvern waits while you push the code via API.
...
**Best for:** Corporate environments where 2FA codes are delivered to systems you control and can expose via webhook.

Instead of pushing codes to Skyvern, Skyvern polls your endpoint until a code is available.
...
| Polling timeout | 10 minutes | Maximum wait time before failing |
