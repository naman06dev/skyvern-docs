...
2. Check workflow parameter configuration—ensure the credential parameter is linked to a Login block
...
1. **Skyvern handles timing automatically** — If a TOTP code has less than 20 seconds remaining, Skyvern waits for the next code
...
<Note>
For login flows with credentials, use the `login()` method which handles credential retrieval automatically. The `run_task()` method is for general automation without credential management.
</Note>
...
| `CredentialNotFound` | Referenced credential ID doesn't exist | Verify credential ID and organization access |
| `VaultConnectionFailed` | Couldn't connect to external vault | Check vault credentials and network connectivity |
| `InvalidCredentialType` | Credential type doesn't match expected | Ensure you're using password credential for logins |
