...
description: Use the Skyvern TypeScript SDK to execute, monitor, and manage multi-step workflows with the runWorkflow method, including parameter passing and browser session reuse.
...
For conceptual background, see [Build a Workflow](/developers/multi-step-automations/build-a-workflow).
...
| `body.browser_session_id` | `string` | No | `undefined` | Run inside an existing [browser session](/developers/optimization/browser-sessions). |
| `body.browser_profile_id` | `string` | No | `undefined` | Load a [browser profile](/developers/optimization/browser-profiles) (cookies, storage) into the session. |
