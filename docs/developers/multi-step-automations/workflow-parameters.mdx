...
description: Full reference for all workflow creation and run parameters including title, definition, proxy location, webhooks, browser sessions, browser profiles, and credential parameters.
slug: developers/multi-step-automations/workflow-parameters
keywords:
  - workflow_id
  - parameters
  - proxy_location
...
Array of blocks that execute in sequence. See [Workflow Blocks Reference](/developers/multi-step-automations/workflow-blocks-reference) for all block types.
...
See [Proxy & Geo Targeting](/developers/going-to-production/proxy-geolocation) for all available locations.
...
See [Proxy & Geo Targeting](/developers/going-to-production/proxy-geolocation) for more details.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="hammer"
    href="/developers/multi-step-automations/build-a-workflow"
  >
