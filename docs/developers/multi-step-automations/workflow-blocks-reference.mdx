...
description: Complete reference for all Skyvern workflow block types including navigation, action, extraction, login, validation, conditional, for loop, file download, text prompt, code, HTTP request, and more.
slug: developers/multi-step-automations/workflow-blocks-reference
keywords:
  - navigation block
  - action block
  - extract block
...
<Info>
For guidance on which engine to choose (`skyvern-1.0` vs `skyvern-2.0`), see [Engine Selection](/developers/running-automations/task-parameters#engine).
</Info>
