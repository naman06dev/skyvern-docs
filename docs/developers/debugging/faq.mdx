...
Parameters are available in prompts using `{{ parameter_name }}` syntax. See [Workflow Parameters](/developers/multi-step-automations/workflow-parameters) for details.
</Accordion>
...
- **API** — Use the [scheduling API](/developers/multi-step-automations/scheduling-workflows) to create, update, enable/disable, and delete schedules programmatically
