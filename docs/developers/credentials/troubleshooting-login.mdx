...
```bash
curl -X GET "https://api.skyvern.com/v1/developers/credentials/cred_xyz789" \
...
```bash
curl -X GET "https://api.skyvern.com/v1/developers/credentials/totp?totp_identifier=user@example.com" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
...
```bash
curl -X GET "https://api.skyvern.com/v1/developers/credentials/YOUR_CREDENTIAL_ID" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
...
```bash
curl -X GET "https://api.skyvern.com/v1/developers/credentials/totp?totp_identifier=YOUR_IDENTIFIER" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
