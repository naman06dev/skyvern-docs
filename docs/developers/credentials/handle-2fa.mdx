...
<Steps>
  <Step title="Create a Zapier Zap">
    ...
    8. In the "Configure", set up these fields to make a POST request to Skyvern's TOTP API:
       - **URL:** `https://api.skyvern.com/v1/developers/credentials/totp`
       - **Payload Type:** `json`
       - **Data:**
...
<Steps>
  ...
  <Step title="Create a Twilio Function to forward codes">
    ...
    exports.handler = async function(context, event, callback) {
      ...
      await axios.post('https://api.skyvern.com/v1/developers/credentials/totp', {
  ...
  <Step title="Test it">
    ...
    ```bash
    curl -X GET "https://api.skyvern.com/v1/developers/credentials/totp?totp_identifier=YOUR_TWILIO_NUMBER" \
      -H "x-api-key: $SKYVERN_API_KEY"
    ```
  </Step>
</Steps>
