...
```bash
curl -X POST "https://api.skyvern.com/v1/developers/credentials/onepassword/create" \
  -H "x-api-key: $SKYVERN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api.skyvern.com/v1/developers/credentials/azure_credential/create" \
  -H "x-api-key: $SKYVERN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "credential": {
...
```bash
curl -X POST "https://api.skyvern.com/v1/developers/credentials/custom_credential/create" \
  -H "x-api-key: $SKYVERN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "config": {
...
```bash cURL
curl -X POST "https://api.skyvern.com/v1/developers/credentials/cred_xyz789/delete" \
  -H "x-api-key: $SKYVERN_API_KEY"
```
</CodeGroup>
