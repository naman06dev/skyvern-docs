...
slug: developers/going-to-production/captcha-bot-detection
keywords:
  - CAPTCHA
  - reCAPTCHA
  - hCaptcha
...
## Use [`error_code_mapping`](/developers/going-to-production/error-handling#step-3-use-error_code_mapping)
...
<Info>
For more on error code mapping and handling failures programmatically, see [Error Handling](/developers/going-to-production/error-handling).
</Info>
...
- **Locale and timezone**: Automatically matched to the proxy location (see [Proxy & Geolocation](/developers/going-to-production/proxy-geolocation))
...
- **Reuse browser sessions for multi-step flows**: Creating a fresh browser for every step looks suspicious. A persistent session maintains cookies, cache, and history. See [Browser Sessions](/developers/optimization/browser-sessions).
