...
description: Connect external credential vaults like Bitwarden, 1Password, Azure Key Vault, or a custom webhook API to the Skyvern Cloud UI so workflows can pull secrets at runtime.
...
@app.get("/api/v1/developers/credentials/{credential_id}")
...
@app.delete("/api/v1/developers/credentials/{credential_id}")
