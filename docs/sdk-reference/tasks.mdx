...
description: Create, run, and monitor browser automation tasks using the Skyvern Python SDK. Covers run_task(), get_run(), cancel_run(), get_run_timeline(), get_run_artifacts(), and task configuration options.
...
<Tip>
For production, prefer `wait_for_completion=True` or [webhooks](/developers/going-to-production/webhooks) over manual polling.
</Tip>
